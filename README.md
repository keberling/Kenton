...
Interactive upload site for field techs to drop project photos. GPS metadata from each image is extracted automatically, and photos are matched to job sites within ~100 m of the geocoded address — or soft-matched to the nearest site within ~2 miles when no other deployment is within ~1 mile of the capture location. Captures farther than ~2 miles from every deployment stay in the match queue.
...
3. **Auto-match on upload** — strict match within ~100 m, or soft match to the nearest site within ~2 mi when captures are isolated from other deployments (~1 mi cushion).
4. **Auto-match on site create** — when a new deployment is registered, the match queue is rescanned and nearby unassigned photos are routed automatically.
...
| `SITE_MAX_MATCH_DISTANCE_M` | `3219` (~2 mi) | Maximum distance for any auto-match; farther captures stay in the queue |
